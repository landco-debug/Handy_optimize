...
2. При первом входе Handy лениво скачивает официальный `codex-app-server` версии 0.155.0 для Apple Silicon.
3. SHA-256 архива проверяется до распаковки.
4. Handy вызывает `account/login/start` с `type = chatgptDeviceCode`.
5. Пользователь получает URL и одноразовый код, открывает страницу ChatGPT и подтверждает вход.
6. Handy ждёт `account/login/completed`, затем проверяет аккаунт через `account/read`.

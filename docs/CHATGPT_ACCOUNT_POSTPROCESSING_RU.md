...
## Горячие клавиши отдельных запросов

Сохранённому запросу постобработки можно назначить собственную горячую клавишу. Такое нажатие запускает обычную диктовку, но для этой конкретной записи использует привязанный запрос независимо от глобально выбранного запроса.

Критическое правило: горячая клавиша запроса **не переключает модель транскрипции**. Распознавание всегда выполняется моделью, которая в этот момент выбрана в Handy/меню моделей. Привязка меняет только prompt постобработки.

Внутренний id динамической привязки: `post_process_prompt:<prompt_id>`.
